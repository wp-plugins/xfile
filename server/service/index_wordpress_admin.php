...
$XAPP_WP_PATH = realpath(
		$XCOM_ROOT . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..'
	) . DIRECTORY_SEPARATOR;//our plugin absolute directory
...
if ($XAPP_FILE_START_PATH != null) {
	if ($XAPP_FILE_START_PATH[0] != '/') {
		...
		if (file_exists(realpath($XAPP_FILE_START_PATH))) {
			$repositoryRoot = $XAPP_FILE_START_PATH;
		}
	}
}
...
if (!file_exists($XAPP_VFS_CONFIG_PATH)) {
	throw new Xapp_Util_Exception_Storage('unable to read storage from file: ' . $XAPP_VFS_CONFIG_PATH, 1640104);
}
...
$xFileRepositoryRoot = XApp_Variable_Mixin::replaceResourceVariables(
	$repositoryRoot,
	array(
		'root' => $repositoryRoot,
...
try {
	...
	$opt = array(
		...
		XApp_Commander_Bootstrap::GATEWAY_CONF => array(
			...
			Xapp_Rpc_Gateway::ALLOW_IP => XApp_Wordpress_Parameter_Helper::getGatewayOption(
				$REDUX_OPTIONS,
				Xapp_Rpc_Gateway::ALLOW_IP
			),
			Xapp_Rpc_Gateway::DENY_IP => XApp_Wordpress_Parameter_Helper::getGatewayOption(
				$REDUX_OPTIONS,
				Xapp_Rpc_Gateway::DENY_IP
			),
			Xapp_Rpc_Gateway::ALLOW_HOST => XApp_Wordpress_Parameter_Helper::getGatewayOption(
				$REDUX_OPTIONS,
				Xapp_Rpc_Gateway::ALLOW_HOST
			),
			Xapp_Rpc_Gateway::DENY_HOST => XApp_Wordpress_Parameter_Helper::getGatewayOption(
				$REDUX_OPTIONS,
				Xapp_Rpc_Gateway::DENY_HOST
			)
		),
