...
//                self::add_ls_file_information($this->toRealPath($path.$item_name),$item,$options[XApp_File_Utils::OPTION_DIR_LIST_FIELDS]);
	...
	private function send($fileName,$content,$options=Array()) {
        ...
        if ($options[XApp_File_Utils::OPTION_SEND]===true) {
            ...
            //$limit=intval($options[XApp_File_Utils::OPTION_SIZE_LIMIT])*1024*1024; // Convert limit to bytes
            /*
            if (strlen($content)>$limit) {
                // chunk file

                $chunk_size=intval($options[XApp_File_Utils::OPTION_CHUNK_SIZE])*1024*1024; // Convert chunk size to bytes
    ...
    private function importFly(){
        ...
        /*
                spl_autoload_register(function($class) {
                    ...
                    $location = XAPP_BASEDIR . 'lib'  . '/' . str_replace('\\', '/', $class) . '.php';
