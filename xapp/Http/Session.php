	...
	private function configure(array $config)
	{
		...
		foreach ($config as $key => $value) {
			...
			if ($value === NULL || ini_get("session.$key") == $value) { // intentionally ==
				...
				if (defined('SID')) {
					throw new XApp_InvalidStateException("Unable to set 'session.$key' to value '$value' when session has been started" . ($this->started ? "." : " by session.auto_start or session_start()."));
				}
