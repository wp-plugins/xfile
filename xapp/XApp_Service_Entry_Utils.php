...
/*$ht = realpath(ABSPATH . DIRECTORY_SEPARATOR . '.htaccess');*/
/*echo(file_get_contents(realpath(ABSPATH . DIRECTORY_SEPARATOR . '.htaccess')));*/
...
function xapp_setup_language($lang=null){
    ...
    if(!function_exists('XAPP_TEXT')){
        ...
        {
            $langToUse = $lang !==null ? $lang : JLanguage::getInstance(null)->getDefault();//;XApp_Service_Entry_Utils::getBrowserDefaultLanguage();
...
class XApp_Service_Entry_Utils {
    ...
    static function getXCommanderOperation(){
        ...
        switch($smdMethod){
            case 'copy':return XC_OPERATION_COPY_STR;
            case 'move':return XC_OPERATION_MOVE_STR;
            case 'deleteItems':return XC_OPERATION_DELETE_STR;
            case 'getContent':return XC_OPERATION_READ_STR;
            case 'setContent':return XC_OPERATION_EDIT_STR;
            case 'mkdir':return XC_OPERATION_NEW_DIRECTORY_STR;
            case 'mkfile':return XC_OPERATION_NEW_FILE_STR;
            case 'compress':return XC_OPERATION_COMPRESS_STR;
            case 'rename':return XC_OPERATION_READ_STR;
            case 'fileUpdate':return XC_OPERATION_FILE_UPDATE_STR;
            case 'download':return XC_OPERATION_DOWNLOAD_STR;
        }
    ...
    public static function includeXAppCore(){
        ...
        if(!defined('XAPP')){
            /*require_once (self::adjustPaths(XAPP_BASEDIR.'/autoload.php'));*/
            /*require_once (self::adjustPaths(XAPP_BASEDIR.'/autoload.php'));*/
    ...
    public static function includeXAppConnectCore(){
        ...
        include(self::adjustPaths(XAPP_BASEDIR . "connect/FakePlugin.php"));//Fake plugin will emulate a RPC plugin for older versions of XApp-Connect-Types.
        include(self::adjustPaths(XAPP_BASEDIR . "connect/CustomTypeManager.php"));//Sync and tools to xapp-studio.com !
        include(self::adjustPaths(XAPP_BASEDIR . "connect/PluginManager.php"));//Sends Messages to ./connect/Joomla/* or /connect/wordpress
        include(self::adjustPaths(XAPP_BASEDIR . "connect/filter/Filter.php"));//base filter class
        include(self::adjustPaths(XAPP_BASEDIR . "connect/filter/Schema.php"));//schema filter (Supports : Inline PHP scripting from client : Applies Result Schema on MySQL or Class queries)
    ...
    public static function isDebug(){
        ...
        if(strpos($pageURL,'indexDebug.php')!==false){
            return true;
        }
