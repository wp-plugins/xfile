...
//		}
	...
	createResource: function(fullPath,  isFolder, parent,cb){
		...
		var length = !isFolder? namesplit.length-1 : namesplit.length;
			for(var i=0;i<length;i++){
				...
				if(folder!=null){
					...
					parent = parent.createResource(namesplit[i],true,false,cb);
				}
			}
			if(!isFolder){
				parent = parent.createResource(namesplit[namesplit.length-1],isFolder,false,cb);
			}
		return parent;
	},
	...
	createProject: function(params) {
        ...
        return workbench.service.serviceObject[workbench.serviceClass]['createProject'](
            params.newProjectName,params.projectToClone,!!params.eclipseSupport,params.projectTemplateName);
        /*return d;*/
...
//		console.log("parent:" + parent + " children :" + newChildrenList);
	...
	findResource: function(name, ignoreCase, inFolder, workspaceOnly){
        ...
        //console.log('finding resource : ' + name + ' in folder : ' + inFolder);
		var seg1 = 0,segments;
...
//				resource.getChildrenSync(function(){}, true);
            ...
            console.log('	found resource in cache : ' + name,found);
            //console.dir(found);
			return found;
		}
		...
		{
            console.log('find resource : ' + name);;
			var response = Runtime.serverJSONRequest({
				url: "cmd/findResource.php",
				content:{
					path: name,
            ...
            console.log('find resource on server : ' + name);
				...
				foundResources = response.map(function(foundFile) {
					...
					for (var j=0;j<foundFile.parents.length;j++) {
						if (!loadResource._isLoaded) {
                            ...
                            //xasync
                            loadResource.setChildren(foundFile.parents[j].members);
						}
