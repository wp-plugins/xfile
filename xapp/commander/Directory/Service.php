...
class XCOM_Directory_Service extends XIDE_Directory_Service
{
	...
	public function put()
	{
		...
		if (array_key_exists('mount', $vars)) {
			$mount = XApp_Path_Utils::getMount(XApp_Path_Utils::decodeSecureMagic($vars['mount']));
		}
		if ($dstIn === '.') {
			$dstIn = '/';
		}
		...
		foreach ($fileVars as $boxName => $boxData) {
			...
			if ($this->isLocal($mount, $this->getFSResources())) {
				...
				if (isSet($boxData["input_upload"])) {
					...
					//error_log('upload from location:  ' . $boxData["tmp_name"]);
				...
				error_log('is remote upload : ' .$mount . ' as ' . $userfile_name . ' destination : ' . $destination);
				/*
				$result = @move_uploaded_file($boxData["tmp_name"], "$destination/" . $userfile_name);
				if (!$result) {
					$realPath = $destination . DS . $userfile_name;
	...
	public function delete($selection, $options, $secure = false)
	{
		...
		$vfs = $this->getFileSystem($mount);
		$vfs->delete($finalSelection, $options, $inclusion, array(), $error, $success);
		return self::toRPCError(count($error) > 0 ? 1 : 0, $error);
	}
	 ...
	 * @return array
	 */
    /*public function ls($basePath = '.', $path = './', $options = null)*/
    public function ls($mount='ws',$path='/',$options=null){
		...
		$vfs = $this->getFileSystem($basePath);
	...
	static function parseFileDataErrors($boxData)
	{
		...
		if ($userfile_error != UPLOAD_ERR_OK) {
			$errorsArray = array();
			$errorsArray[UPLOAD_ERR_FORM_SIZE] = $errorsArray[UPLOAD_ERR_INI_SIZE] = array(
				409,
				"File is too big! Max is" . ini_get("upload_max_filesize")
			);
	...
	public function fileUpdate(){
		...
		//error_log('file update : ' . $srcPath . ' to mount '  . $mount. ' from ' . $remoteUrl);
