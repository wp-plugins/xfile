<?php
/*
   Plugin Name: xappcommander
   Plugin URI: http://wordpress.org/plugins/xappcommander/
   Version: 1.0
   Author: xappcommander
...
function xcom_noticePhpVersionWrong() {
    ...
    echo '<div class="updated fade">' .
        __('Error: plugin "xcom" requires a newer version of PHP to be running.',  'xcom').
        '<br/>' . __('Minimal version of PHP required: ', 'xcom') . '<strong>' . $xcom_minimalRequiredPhpVersion . '</strong>' .
        '<br/>' . __('Your server\'s PHP version: ', 'xcom') . '<strong>' . phpversion() . '</strong>' .
        '</div>';
}
...
function xcom_i18n_init() {
    ...
    load_plugin_textdomain('xcom', false, $pluginDir . '/languages/');
}
...
function xcom_admin_menu() {
    ...
    add_menu_page('XCommander', 'Files', 'administrator', 'xcommander','renderXCOMGUI');
}
