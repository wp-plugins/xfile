		...
		foreach ($serviceList as $serviceConf) {
			...
			if ($instance == null) {
				...
				if (array_key_exists(XApp_Service::PUBLISH_METHODS, $classConf)) {
					...
					//error_log('register service : have published methods : ' . count($publishedMethods));
					...
					if (count($publishedMethods)) {
						...
						/*
						$className::registerMethod('XCF_NodeJS_Debug_Manager','xxx');
					...
					//error_log('service class : ' . $className. ' has no published methods :');
					//error_log('service conf : ' . json_encode($serviceConf));
